...
import Card from "./Card"; import Work from './Work';
import img1 from "../assets/8-removebg-preview.png"; import img2 from "../assets/6-removebg-preview.png";
import img3 from "../assets/2.jpeg"; import img4 from "../assets/4-removebg-preview.png";
import img5 from "../assets/3-removebg-preview.png"; import img6 from "../assets/5-removebg-preview.png";
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const targetUrl = 'https://trainbookingapp.fly.dev/api/v1/users/trips/search';
                const response = await axios.get(proxyUrl + encodeURIComponent(targetUrl), {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (response.data) setDaily(response.data.length);
                setUsers(1200); setStations(45); // قيم افتراضية للمستخدمين والمحطات
            } catch {
                setDaily(0); setUsers(0); setStations(0);
            }
        };
        fetchStats();
    }, []);
  ...
  return (
    <div>
        <div className=" w-full h-[100vh] bgimg bg-cover bg-center flex justify-center items-end pb-5 shadow-2xl mt-16" style={{backgroundImage:`URL(${bgimg})`}}>
            <div><h2 className='sm:text-[40px] font-bold text-white text-[20px] text-center'>Book Your Train Ticket Easily And Secureiy</h2><p className='text-center py-[30px] sm:text-[17px] text-gray-200 font-[400] text-[15px]'>Travel Smarter With Egyption National Railways</p></div>
        </div>
        <div className='w-full h-auto mt-40 sm:h-[120vh]' >
            ...
            <div className='w-[80%] block h-auto m-auto justify-between items-center my-10 sm:flex sm:h-[350px] '>{cards.map(card => <Card key={card.id} {...card}>{card.c}</Card>)}</div>
            <h2 className='text-[#7a1618] text-center text-[45px] font-[700] py-4'>How It Works</h2>
            <div className='w-[80%] sm:h-[150px] m-auto block justify-between items-center my-10 sm:flex h-auto'>{works.map(work => <Work key={work.id} {...work}>{work.c}</Work>)}</div>
        </div>
        <div className='w-[80%] sm:h-[60vh] m-auto sm:flex relative mt-20 block h-auto '>
            <div className='w-full sm:w-[65%] sm:h-[60vh] h-auto '>
                <div className='w-full h-auto flex justify-between items-center sm:h-[35%]'>
                    <div className='w-[30%] h-full '><h2 className='text-center pt-8 flex justify-center items-center text-[40px] text-amber-500 font-bold'>{daily}<span className='text-[20px]'><FaPlus/></span></h2><p className='text-center text-[#6b6b6bc2] text-[20px]'>Daily Trips</p></div>
                    <div className='w-[1px] h-[60px] bg-[#cacaca] shadow'></div>
                    <div className='w-[30%] h-full '><h2 className='text-center pt-8 flex justify-center items-center text-[40px] text-amber-500 font-bold'>{users}<span className='text-[20px]'><FaPlus/></span></h2><p className='text-center text-[#6b6b6bc2] text-[20px]'>Users</p></div>
                    <div className='w-[1px] h-[60px] bg-[#cacaca] shadow'></div>
                    <div className='w-[30%] h-full '><h2 className='text-center pt-8 flex justify-center items-center text-[40px] text-amber-500 font-bold'>{stations}<span className='text-[20px]'><FaPlus/></span></h2><p className='text-center text-[#6b6b6bc2] text-[20px]'>Stations</p></div>
                </div>
                <div className='w-full h-auto sm:h-[65%]'>
                    ...
                    <div className='flex justify-around items-center w-full sm:w-[60%] mt-5 sm:ml-10 '><img src={img8} alt="" className='w-[200px] sm:ml-5 rounded-[8px] shadow-2xl'/><img src={img9} alt="" className='w-[200px] rounded-[8px] shadow-2xl ml-2'/></div>
                </div>
            </div>
